...
public:
    Doors(int x, int y, char symbol)        // Constructor: create door at (x,y) with given symbol
        : x(x), y(y), symbol(symbol),
        requiredKeys(symbol - '0'),         // Convert digit character (e.g. '3') to int 3
        currentKeys(0) {
    }
    ...
    int  getCurrentKeys() const { return currentKeys; }   // Get keys already collected
    void addKey() { ++currentKeys; }        // Add one key to the door’s counter
    bool isOpen() const { return currentKeys >= requiredKeys; } // Check if door is open
